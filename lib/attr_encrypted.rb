...
module AttrEncrypted
  ...
  def attr_encrypted(*attributes)
    ...
    attributes.each do |attribute|
      ...
      define_method(attribute) do
        instance_variable_get("@#{attribute}") || instance_variable_set("@#{attribute}", decrypt_attr(attribute, send(encrypted_attribute_name)))
      end

      define_method("#{attribute}=") do |value|
        send("#{encrypted_attribute_name}=", encrypt_attr(attribute, value))
  ...
  #   end
  #
  #   email = User.decrypt_attr(:email, 'SOME_ENCRYPTED_EMAIL_STRING')
  def decrypt_attr(attribute, encrypted_value, options = {})
  ...
  def encrypt_attr(attribute, value, options = {})
    options = attr_encrypted_attributes[attribute.to_sym].merge(options)
  ...
  def method_missing(method, *arguments, &block)
    if method.to_s =~ /^((en|de)crypt)_(.+)$/ && attr_encrypted?($3)
      send("#{$1}_attr".to_sym, $3, *arguments)
    else
      super
    end
  end
  ...
  module InstanceMethods
    ...
    #  @user.decrypt_attr(:email, 'SOME_ENCRYPTED_EMAIL_STRING')
    def decrypt_attr(attribute, encrypted_value)
      ...
      self.class.decrypt_attr(attribute, encrypted_value, evaluated_attr_encrypted_options_for(attribute))
    end
    ...
    #
    #  @user = User.new('some-secret-key')
    #  @user.encrypt_attr(:email, 'test@example.com')
    def encrypt_attr(attribute, value)
      ...
      self.class.encrypt_attr(attribute, value, evaluated_attr_encrypted_options_for(attribute))
    end
